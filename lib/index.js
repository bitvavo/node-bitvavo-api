...
let api = function Bitvavo() {
  ...
  const publicRequest = async function (url, callback, method = 'GET', data = {}) {
    let options = {
      method: method,
      url: url,
      json: true,
    ...
    if (apiKey !== '') {
      let timestamp = Date.now()
      let sig = await createSignature(apiSecret, timestamp, method, url.replace(base, ''), data)
      options.headers = {
  ...
  const privateRequest = async function (urlEndpoint, urlParams, callback, body = {}, method = 'GET') {
    let url = base + urlEndpoint + urlParams
    let timestamp = Date.now()
    let sig = await createSignature(apiSecret, timestamp, method, urlEndpoint + urlParams, body)
    let options = {
      method: method,
      url: url,
      json: true,
