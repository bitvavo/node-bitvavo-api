...
let api = function Bitvavo() {
      ...
      if (apiKey !== '') {
        doSendPublic(
          ...
          JSON.stringify({
            'action': 'authenticate',
            'key': apiKey,
            'signature': createSignature(apiSecret, timestamp, 'GET', '/websocket'),
            'timestamp': timestamp,
            'window': accessWindow,
          }),
        )
  ...
  const publicRequest = function (url, callback, method = 'GET', data = {}) {
    ...
    if (apiKey !== '') {
      let timestamp = Date.now()
      let sig = createSignature(apiSecret, timestamp, method, url.replace(base, ''), data)
      options.headers = {
  ...
  const privateRequest = function (urlEndpoint, urlParams, callback, body = {}, method = 'GET') {
    ...
    let timestamp = Date.now()
    let sig = createSignature(apiSecret, timestamp, method, urlEndpoint + urlParams, body)
    let options = {
      method: method,
      url: url,
      json: true,
