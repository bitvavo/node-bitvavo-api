...
let api = function Bitvavo() {
   ...
   */
  function authenticate(ws, credentials) {
    doSendPublic(ws, JSON.stringify({ action: 'authenticate', ...credentials }))
  }
   ...
   */
  function doSendPublic(ws, message) {
  ...
  const subscribe = async function (websocketObject) {
    ...
    ws.onopen = async () => {
      await handleSocketOpen.call(websocketObject)
    }
    ws.onmessage = (msg) => {
  ...
  return {
    ...
    /**
     * @typedef BitvavoOptions
     * @prop {string} [apiKey]
     * @prop {string} [apiSecret]
     * @prop {number} [accessWindow]
     * @prop {boolean} [debugging]
     * @prop {string} [restURL]
     * @prop {string} [wsURL]
     * @prop {string} [csrf]
     */
